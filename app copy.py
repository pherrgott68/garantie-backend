...
server = 'PHILIPPEHER843B' 
database = 'RE_Garanties' 
username_db = 'sa' 
password_db = 'Yameseal68' 
...
def histo(numero_serie):
    cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username_db+';PWD='+ password_db)
    cursor = cnxn.cursor()

    result=[]
    #requete
    query="SELECT top 50 * from RE_HistoSeries where numero_serie like '%" + numero_serie + "%';"
...
def getNomenclatures():
    ...
    result = pd.DataFrame(pd.read_sql(query,cnxn))
    return result.to_json(orient='records')
...
def getNomenclature(Id_nomenclature):
    ...
    Lien_ODOO = pd.DataFrame(pd.read_sql(query,cnxn))
...
def U_Nomenclature():
    ...
    #requete
    if data['isNew']==True:
        query="INSERT INTO RE_Art_Nomenclature VALUES ({0},{1},{2}) ;".format(data['id_nomenclature'],data['id_article'],data['quantite'])
    else:
...
def D_Nomenclature():
    ...
    print(data)
    #requete
    query="Delete RE_Art_Nomenclature where id = {0} ;".format(data)
    print(query)
    result = cursor.execute(query)
    cnxn.commit()
